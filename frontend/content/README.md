...
fontScale: 1.34             # 可选。正文字号倍率，不是绝对字号
...
每级标题左边有一根分段灯条加向右的辉光，颜色和强度逐级递减。
标题不要用方括号标记去做 —— 那套是行内的，做不出真正的标题。

`fontScale` 是**倍率**不是绝对字号 —— 基准字号本身跟着视口缩放（见
`src/components/fonts.css` 的 `--content-font-size`），写死 `20px` 等于让这篇
退出那套缩放，大屏上会重新变得太小。倍率是叠上去的，两者不冲突。
取值夹在 0.8–1.6，超出会 `console.warn` 并按边界处理。

主页也是一篇 md：`content/home.md`，路由是 `/`，不进任何列表。它的
`fontScale: 1.34` 是照着"改成 md 之前那版"的观感定的。
